...
const sourceRoot = join(repoRoot, "squash");
const publicRoots = [
  "index.html",
  "styles.css",
  "app.js",
...
const generatedManifest = ".squash-source.json";
...
const verifySource = ({ allowPlaceholderForm }) => {
  ...
  if (result.status !== 0) {
    throw new Error(`Mila Squash source verification failed:\n${result.stderr || result.stdout}`);
  }
};
...
const assertTarget = async (target) => {
  ...
  if ((await readFile(join(resolvedTarget, "CNAME"), "utf8")).trim() !== "squash.mannamila.com") {
    throw new Error("The target CNAME must be squash.mannamila.com.");
  }
...
const sourceIdentity = () => {
  ...
  const sourceTreeDirty = runGit("status", "--porcelain", "--", "squash").length > 0;
...
const main = async () => {
  ...
  if (identity.sourceTreeDirty && !options.allowDirtySource) {
    throw new Error("The squash source tree is dirty. Promote a reviewed source commit, or use --allow-dirty-source only for an isolated test.");
  }
