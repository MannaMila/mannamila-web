...
const privacyText = privacy.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ");
assert.match(privacyText, /This policy applies to Skald: Odyssey 0\.7\.0/);
assert.match(privacyText, /only if you allow Apple's tracking permission after actively completing a free book/);
assert.match(privacyText, /odyssey_full_unlock_edu/);
